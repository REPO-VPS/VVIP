...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/JurigVPN/izinvps/ipuk/ip | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/JurigVPN/backup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    ...
    git remote add origin https://github.com/JurigVPN/backup
    git push -f https://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@github.com/JurigVPN/backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/JurigVPN/backup/ipuk/$NameUser/$NameUser.zip"
sleep 1
